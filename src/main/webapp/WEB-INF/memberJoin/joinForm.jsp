            ...
            <div class="mb-3">
              <div class="form-check">
                ...
                <label class="form-check-label" for="terms-conditions">
                  통합 이용약관, 개인정보수집 및 이용,
                  <a href="javascript:void(0);">위치정보이용약관 (선택) 에모두 동의 합니다.</a>
                </label>

              </div>
            </div>
            <div>
              <span id="final_boxChk" style="display: none; color: red; margin-bottom: 10px;">약관에 동의해주세요.</span>
            </div>
...
<script>
  ...
  document.querySelector("#emailBtn").addEventListener("click",function (e){
      ...
      if(document.querySelector("#email").value == "" || strongEmail(document.querySelector("#email").value) === false){
        ...
        $.ajax({
          ...
          type:"GET",
          url:"/memberJoin/email?email=" + emailValue,
  ...
  $(document).ready(function (){
    document.querySelector("#joinBtn").addEventListener("click", function (){
      ...
      //checkbox 선택 확인
      let check = document.querySelector("#terms-conditions");
       if(check.checked) {
         document.querySelector("#final_boxChk").style.display = 'none';
         checkBoxCheck = true;
       } else {
         document.querySelector("#final_boxChk").style.display = 'block';
         checkBoxCheck = false;
       }

      let frm = document.querySelector("#formAuthentication");
      if(checkBoxCheck&&idCheck&&idDutCheck&&pwCheck&&pwReCheck&&nameCheck&&emailCheck&&emailNumCheck&&addressCheck&&phoneCheck){
          frm.method = "post";
          frm.action = "/memberJoin/join";
          frm.submit();
      }
