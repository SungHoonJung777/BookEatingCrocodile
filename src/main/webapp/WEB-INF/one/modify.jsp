...
<div class="container">
    ...
    <div class="layout-container layout-content-navbar">
        <!-- Menu -->
        <jsp:include page="../member/nav.jsp"/>
                    ...
                    <div class="row">
                        <div class="col-xl">
                            <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    <form action="/one/modify" method="post">
                                        <input type="hidden" name="member_id" value="${qnaDTO.member_id}">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select id="qna_select" name="qna_select" onchange="goList()" class="border-0 form-select-sm bg-light me-3">
                                                <option value="기타" <c:if test="${qnaDTO.qna_select eq '기타'}"> selected</c:if>>기타</option>
                                                <option value="상품문의" <c:if test="${qnaDTO.qna_select eq '상품문의'}"> selected</c:if>>상품문의</option>
                                                <option value="주문문의" <c:if test="${qnaDTO.qna_select eq '주문문의'}"> selected</c:if>>주문문의</option>
                                                <option value="주문문의" <c:if test="${qnaDTO.qna_select eq '배송문의'}"> selected</c:if>>배송문의</option>
                                        ...
                                        <button type="reset" class="btn btn-secondary" onclick="location.href='/one/view?qna_idx=${qnaDTO.qna_idx}'">게시글로 돌아가기</button>
