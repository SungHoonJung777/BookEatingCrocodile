                    ...
                    <table class="table">
                        ...
                        <tr>
                            <th  class="align-middle">
                                <div class="d-flex justify-content-center">
                                    <input type="checkbox" class="form-check-input mt-0" id="checkbox${i.count}" name="cart_idx" onclick="checkproduct();" value="${list.cart_idx}">
                                </div>
                            </th>
                            <th scope="row">
                            ...
                            <td>
                                <div class="input-group quantity mt-4" style="width: 100px;">
                                    <div class="input-group-btn">
                                        <button class="btn btn-sm btn-minus rounded-circle bg-light border" type="button" onclick="minus(${i.count}, ${list.pro_price}, ${list.cart_idx})">
                                            <i class="fa fa-minus"></i>
                                        </button>
                                    </div>
                                    <input type="text" class="form-control form-control-sm text-center border-0" value="${list.pro_quantity}" id="product${i.count}" readonly>
                                    <div class="input-group-btn">
                                        <button class="btn btn-sm btn-plus rounded-circle bg-light border" type="button" onclick="plus(${i.count}, 500, ${list.pro_price}, ${list.cart_idx})">
                            ...
                            <td>
                                <button class="btn btn-md rounded-circle bg-light border mt-4" onclick="cartout(${list.cart_idx})" type="button">
                ...
                <div class="row g-4 justify-content-end">
                    ...
                    <div class="col-sm-8 col-md-7 col-lg-6 col-xl-4">
                        <div class="bg-light rounded">
                            ...
                            <button class="btn border-secondary rounded-pill px-4 py-3 text-primary text-uppercase mb-4 ms-4" type="submit" disabled id="checkout">결제하기</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function plus(idx, amount, price, cart_idx){
        let quantity = document.getElementById("product"+idx).value;
        if(parseInt(quantity)+1 <= amount) {
            $.ajax({
                type: "POST",            // HTTP method type(GET, POST) 형식이다.
                url: "/member/cartplus",      // 컨트롤러에서 대기중인 URL 주소이다.
                data: {
                    cart_idx: cart_idx
                },            // Json 형식의 데이터이다.
                success: function (result) { // 비동기통신의 성공일경우 success콜백으로 들어옵니다. 'res'는 응답받은 데이터이다.
    ...
    function checkAll(el){
        let checkBoxes  = document.getElementsByName("cart_idx");
    ...
    function checkproduct(){
        let totalval = document.getElementsByName("cart_idx");
        let total_price = 0;
        let count = 0;
        let uncheck = 0;
        ...
        if(uncheck == count) {
            ...
            document.getElementById("shipping").innerHTML = 3000;
            document.getElementById("checkout").disabled=false;
            document.getElementById("total_price").value = total_price+3000;
        }
